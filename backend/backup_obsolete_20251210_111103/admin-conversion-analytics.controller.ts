  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const [
        ...
        (supabaseService as any).serviceClient.rpc(
          'get_time_to_conversion_metrics',
          {
            p_shop_domain: shop,
      ...
      const kpis = {
        ...
        avgOrderValue:
          conversionMetrics.attributedOrders > 0
            ? conversionMetrics.attributedRevenue /
              conversionMetrics.attributedOrders
            : 0,
        roas:
          conversionMetrics.attributedRevenue /
          Math.max(conversionMetrics.totalRecommendations * 0.01, 1), // Assuming $0.01 cost per recommendation
        timeToConversion:
          timeToConversionData.data?.[0]?.avg_time_to_purchase_hours || 0,
      };
      ...
      const previousMetrics =
        await conversionTrackingService.getConversionMetrics(
          shop as string,
          previousStartDate,
          startDate,
          false
      ...
      const trends = {
        recommendations: calculateTrend(
          conversionMetrics.totalRecommendations,
          previousMetrics.totalRecommendations
        ),
        revenue: calculateTrend(
          conversionMetrics.attributedRevenue,
          previousMetrics.attributedRevenue
        ),
    ...
    try {
      ...
      const summary = {
        last7Days: {
          ...
          conversionRate:
            weeklyMetrics.conversionRates.recommendationToPurchase,
        },
        last30Days: {
          ...
          conversionRate:
            monthlyMetrics.conversionRates.recommendationToPurchase,
        },
        allTime: {
          ...
          conversionRate:
            allTimeMetrics.conversionRates.recommendationToPurchase,
        },
      };
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { shop, from, to, granularity = 'daily' } = req.query;
      ...
      const { data: timeSeriesData, error } = await (
      ...
      if (error) {
        throw new AppError(
          `Failed to fetch time series data: ${error.message}`,
          500
        );
      }

      // Get attribution conflicts and data quality metrics
      const { data: attributionConflicts } = await (
        supabaseService as any
      ).serviceClient.rpc('detect_attribution_conflicts', {
        p_shop_domain: shop,
      ...
      const { data: positionEffectiveness } = await (
        supabaseService as any
      ).serviceClient.rpc('get_position_effectiveness', {
        p_shop_domain: shop,
        p_date_from: fromDate.toISOString().split('T')[0],
        p_date_to: toDate.toISOString().split('T')[0],
      });
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const totalCost =
        metrics.totalRecommendations * assumedCostPerRecommendation;
      const roi =
        totalCost > 0
          ? ((metrics.attributedRevenue - totalCost) / totalCost) * 100
          : 0;
      ...
      const impact = {
        ...
        roi,
        valuePerRecommendation:
          metrics.totalRecommendations > 0
            ? metrics.attributedRevenue / metrics.totalRecommendations
            : 0,
        topPerformingTypes: Object.entries(
          metrics.recommendationTypePerformance
        )
        ...
        insights: [
          ...
          },
          {
            ...
            trend:
              metrics.conversionRates.recommendationToPurchase > 0.05
                ? 'positive'
                : 'neutral',
          },
          {
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const exportData = {
        ...
        summary: {
          ...
          totalRecommendations:
            dailyFunnel.data?.reduce(
              (sum: number, day: any) => sum + (day.total_recommendations || 0),
              0
            ) || 0,
          totalRevenue:
            dailyFunnel.data?.reduce(
              (sum: number, day: any) =>
                sum + parseFloat(day.attributed_revenue || 0),
              0
            ) || 0,
        },
      ...
      if (format === 'csv') {
        ...
        res.setHeader(
          'Content-Disposition',
          `attachment; filename="conversion-analytics-${shop}-${fromDate.toISOString().split('T')[0]}-${toDate.toISOString().split('T')[0]}.csv"`
        );
        res.send(csv);
      } else {
        // Return JSON
