...
import { AppError, TenantPlan, TenantStatus, TENANT_PLAN_LIMITS } from '@/types';
...
router.get('/stats', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const stats = {
      ...
      totalMessages: tenants.reduce((sum, t) => sum + t.monthly_messages_used, 0),
...
router.get('/', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    logger.info('Fetching tenants list', { page: pageNum, limit: limitNum, status, plan });
 ...
 */
router.get('/:shopDomain', async (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.post('/', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    try {
      const { error: settingsError } = await (supabaseService as any).serviceClient
        .from('app_settings')
        .insert({
          shop_domain,
          chat_enabled: true,
 ...
 * Update a tenant
 */
router.put('/:shopDomain', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { shopDomain } = req.params;
    ...
    if (plan && plan !== existingTenant.plan) {
      const result = await tenantService.updatePlan(shopDomain, plan as TenantPlan);
      if (result) {
 ...
 */
router.put('/:shopDomain/status', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { shopDomain } = req.params;
    ...
    const validStatuses: TenantStatus[] = ['active', 'suspended', 'cancelled', 'trial'];
    ...
    const success = await tenantService.updateStatus(shopDomain, status as TenantStatus);
 ...
 */
router.delete('/:shopDomain', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { shopDomain } = req.params;
    ...
    if (hard === 'true') {
      ...
      const success = await tenantService.updateStatus(shopDomain, 'cancelled');
      if (!success) {
    ...
    res.json({
      success: true,
      message: hard === 'true' ? 'Tenant deleted permanently' : 'Tenant cancelled',
    });
  } catch (error) {
 ...
 */
router.get('/:shopDomain/usage', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { shopDomain } = req.params;
 ...
 */
router.post('/:shopDomain/reset-usage', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { shopDomain } = req.params;
    ...
    const { error } = await (supabaseService as any).serviceClient
      .from('tenants')
      .update({ monthly_messages_used: 0, updated_at: new Date().toISOString() })
      .eq('shop_domain', shopDomain);
 ...
 */
router.get('/plans/info', async (req: Request, res: Response, next: NextFunction) => {
  try {
    res.json({
      success: true,
