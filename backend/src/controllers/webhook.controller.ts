...
const verifyWebhook = (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    (req as any).webhookData = typeof body === 'string' ? JSON.parse(body) : body;
...
router.post('/products/create', verifyWebhook, async (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.post('/products/update', verifyWebhook, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const shopDomain = (req as any).shopDomain;
    const product = (req as any).webhookData;
...
router.post('/products/delete', verifyWebhook, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const shopDomain = (req as any).shopDomain;
    const product = (req as any).webhookData;
...
router.post('/app/uninstalled', verifyWebhook, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const shopDomain = (req as any).shopDomain;
...
router.post('/:topic', verifyWebhook, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const shopDomain = (req as any).shopDomain;
...
async function logWebhookEvent(shopDomain: string, topic: string, payload: any): Promise<void> {
  try {
    const { error } = await (supabaseService as any).serviceClient
      .from('webhook_events')
      .insert({
