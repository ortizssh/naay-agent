...
router.get('/funnel', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!['daily', 'weekly', 'monthly'].includes(breakdown as string)) {
      throw new AppError('Invalid breakdown period. Use daily, weekly, or monthly', 400);
    }

    // Get funnel data from the daily view
    const { data: funnelData, error } = await (conversionTrackingService as any)
    ...
    if (error) {
      throw new AppError(`Failed to fetch funnel data: ${error.message}`, 500);
    }
    ...
    const totalRecommendations = groupedData.reduce((sum, day) => sum + (day.total_recommendations || 0), 0);
    const totalCartConversions = groupedData.reduce((sum, day) => sum + (day.cart_conversions || 0), 0);
    const totalPurchaseConversions = groupedData.reduce((sum, day) => sum + (day.purchase_conversions || 0), 0);
    const totalRevenue = groupedData.reduce((sum, day) => sum + parseFloat(day.attributed_revenue || 0), 0);
    ...
    const funnelSteps = [
      ...
      },
      {
        ...
        rate: totalRecommendations > 0 ? totalCartConversions / totalRecommendations : 0
      },
      {
        step: 'purchases',
        label: 'Completed Purchases',
        count: totalPurchaseConversions,
        rate: totalRecommendations > 0 ? totalPurchaseConversions / totalRecommendations : 0
      }
    ];
    ...
    const response: APIResponse = {
      ...
      data: {
        ...
        funnelSteps,
        totalRevenue,
        avgRevenuePerRecommendation: totalRecommendations > 0 ? totalRevenue / totalRecommendations : 0,
        avgRevenuePerPurchase: totalPurchaseConversions > 0 ? totalRevenue / totalPurchaseConversions : 0,
...
router.get('/top-products', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const { data: topProducts, error } = await (conversionTrackingService as any)
      .supabaseService.serviceClient
    ...
    if (error) {
      throw new AppError(`Failed to fetch top products: ${error.message}`, 500);
    }
 ...
 */
router.get('/recommendation-types', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const { data: typePerformance, error } = await (conversionTrackingService as any)
      .supabaseService.serviceClient
    ...
    if (error) {
      throw new AppError(`Failed to fetch recommendation type performance: ${error.message}`, 500);
    }
 ...
 */
router.get('/time-to-conversion', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { shop, from, to } = req.query;
    ...
    const { data: timeMetrics, error } = await (conversionTrackingService as any)
      .supabaseService.serviceClient
      .rpc('get_time_to_conversion_metrics', {
        p_shop_domain: shop,
        p_date_from: fromDate.toISOString().split('T')[0],
        p_date_to: toDate.toISOString().split('T')[0]
      });
    ...
    if (error) {
      throw new AppError(`Failed to fetch time to conversion metrics: ${error.message}`, 500);
    }
 ...
 */
router.get('/cohort-analysis', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { 
      shop, 
      from, 
    ...
    if (!['daily', 'weekly', 'monthly'].includes(cohort_period as string)) {
      throw new AppError('Invalid cohort period. Use daily, weekly, or monthly', 400);
    }

    const { data: cohortData, error } = await (conversionTrackingService as any)
      .supabaseService.serviceClient
      .rpc('get_conversion_cohort_analysis', {
        p_shop_domain: shop,
        p_date_from: fromDate.toISOString().split('T')[0],
        p_date_to: toDate.toISOString().split('T')[0],
    ...
    if (error) {
      throw new AppError(`Failed to fetch cohort analysis: ${error.message}`, 500);
    }
 ...
 */
router.get('/session-funnel/:sessionId', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const { data: sessionMetrics, error } = await (conversionTrackingService as any)
      .supabaseService.serviceClient
      .rpc('get_session_funnel_metrics', {
        p_session_id: sessionId,
        p_shop_domain: shop
      });

    if (error) {
      throw new AppError(`Failed to fetch session metrics: ${error.message}`, 500);
    }

    // Get detailed session data
    const { data: sessionRecommendations, error: recError } = await (conversionTrackingService as any)
      .supabaseService.serviceClient
    ...
    if (recError) {
      throw new AppError(`Failed to fetch session recommendations: ${recError.message}`, 500);
    }
 ...
 */
router.post('/calculate-attribution', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { 
      shop, 
 ...
 */
router.post('/generate-snapshot', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { 
      shop, 
      date_from, 
    ...
    if (!shop || !date_from || !date_to) {
      throw new AppError('Shop domain, date_from, and date_to are required', 400);
    }

    if (!['daily', 'weekly', 'monthly'].includes(snapshot_type)) {
      throw new AppError('Invalid snapshot type. Use daily, weekly, or monthly', 400);
    }
 ...
 */
router.delete('/cleanup/:shop', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (retentionDays < 1 || retentionDays > 3650) {
      throw new AppError('Retention days must be between 1 and 3650 (10 years)', 400);
    }
