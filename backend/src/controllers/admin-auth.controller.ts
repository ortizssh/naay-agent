...
router.post('/register', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { firstName, lastName, email, password, company, plan, userType } = req.body;
    ...
    const { data: existingUser } = await (supabaseService as any).serviceClient
      .from('admin_users')
      .select('id')
    ...
    logger.info('Admin user registered successfully', { userId: user.id, email });
