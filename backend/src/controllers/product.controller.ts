...
router.get('/:productId/similar', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const shop = (req as any).shop;
    ...
    if (error) {
      throw new AppError(`Failed to get similar products: ${error.message}`, 500);
    }
...
router.get('/:productId', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const { data: product, error } = await (supabaseService as any).serviceClient
      .from('products')
      .select(`
        *,
        product_variants (*)
