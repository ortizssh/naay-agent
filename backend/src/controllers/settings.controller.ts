    ...
    try {
      ...
      if (error && error.code === 'PGRST116') {
        appSettings = {
          ...
          fallback_message:
            'Lo siento, en este momento no puedo ayudarte. Por favor, intenta más tarde.',
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (welcome_message && welcome_message.length > 500) {
        throw new AppError(
          'Welcome message is too long (max 500 characters)',
          400
        );
      }
      ...
      if (fallback_message && fallback_message.length > 500) {
        throw new AppError(
          'Fallback message is too long (max 500 characters)',
          400
        );
      }
      ...
      if (
        max_conversation_history &&
        (max_conversation_history < 1 || max_conversation_history > 200)
      ) {
        throw new AppError(
          'Conversation history must be between 1 and 200',
          400
        );
      }
      ...
      if (typeof chat_enabled === 'boolean')
        updateData.chat_enabled = chat_enabled;
      ...
      if (typeof auto_open_chat === 'boolean')
        updateData.auto_open_chat = auto_open_chat;
      if (typeof show_agent_avatar === 'boolean')
        updateData.show_agent_avatar = show_agent_avatar;
      if (typeof business_hours_enabled === 'boolean')
        updateData.business_hours_enabled = business_hours_enabled;
      if (business_hours_start)
        updateData.business_hours_start = business_hours_start;
      if (business_hours_end)
        updateData.business_hours_end = business_hours_end;
      ...
      if (max_conversation_history)
        updateData.max_conversation_history = max_conversation_history;
      if (typeof enable_product_recommendations === 'boolean')
        updateData.enable_product_recommendations =
          enable_product_recommendations;
      if (typeof enable_order_tracking === 'boolean')
        updateData.enable_order_tracking = enable_order_tracking;
      if (typeof enable_analytics === 'boolean')
        updateData.enable_analytics = enable_analytics;
