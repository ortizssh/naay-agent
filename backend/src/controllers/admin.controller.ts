...
router.post('/emergency/activate-widget', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { shop } = req.body;
    ...
    const { data: existingSettings, error: settingsError } = await (supabaseService as any).serviceClient
      .from('app_settings')
      .select('*')
      .eq('shop_domain', shop)
      .single();
    ...
    if (settingsError && settingsError.code === 'PGRST116') {
      ...
      const { data: newSettings, error: createError } = await (supabaseService as any).serviceClient
        .from('app_settings')
        .insert({
          shop_domain: shop,
          chat_enabled: true,
...
router.get('/emergency/widget-status/:shop', async (req: Request, res: Response, next: NextFunction) => {
  try {
