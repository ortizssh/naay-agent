...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const stats = await simpleConversionTracker.getConversionStats(
        shop,
        daysBack
      );
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const { data: activeRecommendations } = await (
        supabaseService as any
      ).serviceClient
        .from('simple_recommendations')
        .select('*')
      ...
      const { data: recentConversions } = await (
        supabaseService as any
      ).serviceClient
        .from('simple_conversions')
        .select('*')
      ...
      const { data: dailyStats } = await (
        supabaseService as any
      ).serviceClient.rpc('get_daily_conversion_stats', {
        p_shop_domain: shop,
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!sessionId || !shopDomain || !productId) {
        throw new AppError(
          'sessionId, shopDomain, and productId are required',
          400
        );
      }
