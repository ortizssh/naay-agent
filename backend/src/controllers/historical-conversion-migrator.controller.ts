...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { shop, daysBack = 30, dryRun = true } = req.body;
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { shop, daysBack = 7, limit = 20 } = req.body;
...
router.delete(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      logger.warn('Clearing historical conversions', {
        shop,
        requestedBy: 'admin',
      });
      ...
      logger.info('Historical conversions cleared', {
        shop,
        deletedCount: count,
      });
...
function analyzeSessionConversions(conversions: any[]): any {
  ...
  return {
    ...
    sessionsWithMultipleConversions: Array.from(sessionMap.values()).filter(
      s => s.conversions > 1
    ).length,
    avgConversionsPerSession:
      Array.from(sessionMap.values()).reduce(
...
function generateRecommendations(result: any): string[] {
  ...
  if (result.summary.conversionRate > 10) {
    recommendations.push(
      'High conversion rate detected - migration would provide valuable historical insights'
    );
  }

  if (result.summary.averageMinutesToConversion < 3) {
    recommendations.push(
      'Very fast conversions - customers are highly responsive to recommendations'
    );
  }

  if (result.processed.conversions > 50) {
    recommendations.push(
      'Significant historical conversion data available - recommend running full migration'
    );
  } else if (result.processed.conversions < 10) {
    recommendations.push(
      'Limited conversion data - consider increasing attribution window or checking recommendation quality'
    );
  }

  if (result.summary.totalRevenue > 1000) {
    recommendations.push(
      'High-value conversions detected - tracking will help optimize AI recommendations'
    );
  }
