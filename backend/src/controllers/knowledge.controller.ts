...
const upload = multer({
  ...
  fileFilter: (_req, file, cb) => {
    ...
    if (allowed.includes(file.mimetype) || file.originalname.match(/\.(txt|md|pdf)$/i)) {
      cb(null, true);
    } else {
 ...
 */
router.get('/', requireClientAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {
 ...
 */
router.post('/', requireClientAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const shopDomain = await getShopDomain(req);
 ...
 */
router.post('/upload', requireClientAuth, upload.single('file'), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const shopDomain = await getShopDomain(req);
    const file = req.file;
 ...
 */
router.get('/:documentId', requireClientAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const shopDomain = await getShopDomain(req);
    const document = await knowledgeService.getDocument(req.params.documentId, shopDomain);
 ...
 */
router.delete('/:documentId', requireClientAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const shopDomain = await getShopDomain(req);
 ...
 */
router.get('/:documentId/status', requireClientAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const shopDomain = await getShopDomain(req);
    const status = await knowledgeService.getDocumentStatus(req.params.documentId, shopDomain);
