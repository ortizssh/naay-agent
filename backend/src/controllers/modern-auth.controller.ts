...
router.post('/refresh', validateSessionToken, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const shop = (req as any).shop;
    const sessionId = (req as any).sessionId;

    logger.info(`Refreshing auth status for shop: ${shop}, session: ${sessionId}`);
...
router.get('/config', validateSessionToken, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const shop = (req as any).shop;
