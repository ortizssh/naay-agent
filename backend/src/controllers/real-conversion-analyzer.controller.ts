...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const analytics = await analyzer.getRealConversionAnalytics(
        shop,
        daysBackNum
      );
      ...
      res.json({
        ...
        data: {
          ...
          period: {
            daysBack: daysBackNum,
            startDate: new Date(
              Date.now() - daysBackNum * 24 * 60 * 60 * 1000
            ).toISOString(),
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const comparison = {
        ...
        insights: {
          effectivenessScore:
            analysis.summary.conversionRate > 0
              ? Math.min(100, analysis.summary.conversionRate * 10)
              : 0,
...
function generateInsights(analysis: any, analytics: any): string[] {
  const insights = [];

  if (analysis.conversionsFound > 0) {
    insights.push(
      `AI recommendations generated ${analysis.conversionsFound} real conversions worth €${analysis.totalRevenue.toFixed(2)}`
    );
  }

  if (analysis.summary.conversionRate > 5) {
    insights.push(
      `High conversion rate of ${analysis.summary.conversionRate.toFixed(1)}% indicates effective AI recommendations`
    );
  } else if (analysis.summary.conversionRate > 0) {
    insights.push(
      `Conversion rate of ${analysis.summary.conversionRate.toFixed(1)}% shows room for improvement in recommendation quality`
    );
  }

  if (analysis.summary.averageMinutesToConversion < 5) {
    insights.push(
      `Fast average conversion time of ${analysis.summary.averageMinutesToConversion.toFixed(1)} minutes shows customers respond quickly to recommendations`
    );
  }

  if (analysis.ordersAnalyzed > 10 && analysis.conversionsFound === 0) {
    insights.push(
      'No conversions detected despite multiple orders - consider reviewing product matching logic or extending attribution window'
    );
  }

  if (analytics.accuracy > 80) {
    insights.push(
      `Model accuracy of ${analytics.accuracy.toFixed(1)}% indicates predictions closely match real conversions`
    );
  }
  ...
  if (analysis.summary.topProducts.length > 0) {
    ...
    insights.push(
      `"${topProduct.productTitle}" is the top converting product with ${topProduct.conversions} conversions`
    );
  }

  return insights.length > 0
    ? insights
    : ['No significant patterns detected in current data'];
}
