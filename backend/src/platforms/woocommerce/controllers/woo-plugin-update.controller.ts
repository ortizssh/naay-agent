...
router.all('/update-info', async (req: Request, res: Response) => {
    try {
        ...
        const siteUrl = requestData.site_url || req.headers['x-kova-site'] || 'unknown';
        const currentVersion = requestData.version || 'unknown';

        console.log(`[Plugin Update Check] Site: ${siteUrl}, Current Version: ${currentVersion}`);

        // Get the download URL - this should point to the latest plugin ZIP
        const baseUrl = process.env.API_URL || process.env.SHOPIFY_APP_URL || 'https://api.kova.ai';
...
router.get('/download', async (req: Request, res: Response) => {
    try {
        ...
        const pluginZipPath = path.join(__dirname, '../../../../public/downloads/kova-agent.zip');

        if (fs.existsSync(pluginZipPath)) {
            // Serve local ZIP file
            res.setHeader('Content-Type', 'application/zip');
            res.setHeader('Content-Disposition', `attachment; filename="kova-agent-${PLUGIN_INFO.version}.zip"`);
