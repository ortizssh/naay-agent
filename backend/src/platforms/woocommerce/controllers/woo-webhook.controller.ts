...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const normalizedProduct = await wooService.getProduct(
        siteUrl,
        product.id.toString()
      );
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      try {
        ...
        const { data: recentSessions } = await (
          ...
          .eq('shop_domain', siteUrl)
          .or(
            `metadata->>customer_email.eq.${order.billing.email},metadata->>customer_id.eq.${order.customer_id}`
          )
          .gte(
            'created_at',
