...
export class ModernShopifyService extends ShopifyService {
   ...
   */
  async getAllProductsWithAuth(shop: string, accessToken: string): Promise<any[]> {
    try {
      ...
      logger.info(`Successfully fetched ${products.length} products with modern auth`);
      return products;
    } catch (error) {
   ...
   */
  async createWebhooksWithAuth(shop: string, accessToken: string): Promise<void> {
    try {
      ...
      logger.info(`Successfully created webhooks with modern auth for shop: ${shop}`);
    } catch (error) {
