...
Extract relevant entities from the message and provide confidence score (0-1).`;
  ...
  private async handleProductSearch(
    ...
    try {
      ...
      const products = await this.supabaseService.searchProducts(
        shop,
        searchQuery,
        queryEmbedding,
        5
      ...
      if (products.length === 0) {
        return {
          messages: [
            `I couldn't find any products matching "${searchQuery}". Could you try describing what you're looking for differently?`,
          ],
          actions: [],
          metadata: {
...
Would you like more details about any of these products or would you like me to add any to your cart?`;
  ...
  private async handleAddToCart(
    ...
    try {
      ...
      return {
        messages: [
          'I had trouble adding items to your cart. Please try again.',
        ],
        actions: [],
        metadata: { error: error.message },
      };
