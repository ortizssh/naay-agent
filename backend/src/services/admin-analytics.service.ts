...
export class AdminAnalyticsService {
  ...
  async getChartAnalytics(
    ...
    try {
      ...
      const conversationCreationLog: Array<{
        sessionId: string;
        creationDate: string;
        dateOnly: string;
      }> = [];
      ...
      const sortedDates = Array.from(sessionFirstMessages.values()).sort(
      ...
      logger.info('Historical conversation date analysis completed', {
        ...
        conversationDateSpread: {
          earliest:
            sortedDates.length > 0 ? sortedDates[0].toISOString() : null,
          latest:
        ...
        dataIntegrityCheck: {
          ...
          invalidDates: Array.from(sessionFirstMessages.values()).filter(date =>
            isNaN(date.getTime())
          ).length,
        },
      });
