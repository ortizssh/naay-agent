...
describe('AIAgentService', () => {
  ...
  beforeEach(() => {
    ...
    mockEmbeddingService =
      EmbeddingService.prototype as jest.Mocked<EmbeddingService>;
    mockSupabaseService =
      SupabaseService.prototype as jest.Mocked<SupabaseService>;
  });
  ...
  describe('processMessage', () => {
    it('should process a product search intent', async () => {
      ...
      // Setup mocks
      mockSupabaseService.searchProducts = jest
        .fn()
        .mockResolvedValue(mockSearchResults);
      mockSupabaseService.getSessionHistory = jest.fn().mockResolvedValue([]);
      mockSupabaseService.saveChatMessage = jest
        .fn()
        .mockResolvedValue({ id: 'msg-1' });
      mockSupabaseService.updateSessionActivity = jest
        .fn()
        .mockResolvedValue(undefined);

      // Mock the private methods
      jest
        .spyOn(service as any, 'analyzeIntent')
        .mockResolvedValue(mockIntentAnalysis);
    ...
    it('should provide fallback response for unknown intents', async () => {
      ...
      jest.spyOn(service as any, 'handleUnknownIntent').mockResolvedValue({
        messages: [
          'Lo siento, no entendí tu mensaje. ¿Puedes ser más específico?',
        ],
        actions: [],
    ...
    it('should handle API errors gracefully', async () => {
      ...
      await expect(
        service.processMessage(mockMessage, mockSessionId, mockShop)
      ).rejects.toThrow('Failed to process message: API Error');
    });
  });
  ...
  describe('Product Search Handler', () => {
    ...
    it('should return formatted product recommendations', async () => {
      ...
      const result = await (service as any).handleProductSearch(
        'zapatos deportivos',
        'test-shop',
        mockIntentAnalysis
      );
  ...
  describe('Cart Operations', () => {
    it('should handle add to cart requests', async () => {
      ...
      const result = await (service as any).handleAddToCart(
        'agrega vestido rojo',
        'test-shop',
        'cart_123',
        mockIntentAnalysis
