...
function WidgetConfig() {
  ...
  const [activeTab, setActiveTab] = useState<'appearance' | 'content' | 'features' | 'ai'>('appearance');
  ...
  const loadConfig = async () => {
    try {
      setLoading(true);
      const [configResponse, codeResponse, aiConfigResponse] = await Promise.all([
        clientApi.getWidgetConfig(),
        clientApi.getWidgetCode(),
        clientApi.getAiConfig(),
      ]);
        ...
        <div className="widget-preview-container">
          <div className="card">
            ...
            {activeTab === 'ai' && (
              <>
                <div className="card-header">
                  <h3 className="card-title">Configuracion de IA</h3>
                </div>
                  ...
                  <button
                    ...
                    onClick={async () => {
                      try {
                        ...
                        setSuccess(true);
                        setTimeout(() => setSuccess(false), 3000);
                        // Reload widget code (endpoint may have changed)
                        const codeResponse = await clientApi.getWidgetCode();
                        if (codeResponse.data?.code) {
                          setWidgetCode(codeResponse.data.code);
                        }
                    ...
                    disabled={savingAi}
                  >
                    {savingAi ? 'Guardando...' : 'Guardar Configuracion IA'}
                  </button>
                </div>
              </>
            )}
